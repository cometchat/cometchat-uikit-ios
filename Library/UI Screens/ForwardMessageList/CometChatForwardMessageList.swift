...
public class CometChatForwardMessageList: UIViewController {
    ...
    override public func loadView() {
        ...
        set(title: "Forward Message", mode: .always)
    }
    ...
    private func addForwardButton(bool: Bool){
        if bool == true {
            let forward = UIBarButtonItem(title: "Forward", style: .done, target: self, action: #selector(self.didForwardButtonPressed))
    ...
    @objc func didForwardButtonPressed() {
        if selectedConversations.count <= 5 {
                                    ...
                                    CometChat.sendTextMessage(message: message, onSuccess: { (message) in
                                        ...
                                        DispatchQueue.main.async {
                                            if let name = (conversation.conversationWith as? User)?.name {
                                                let snackbar: CometChatSnackbar = CometChatSnackbar.init(message: "Unable to forward message to \(name)", duration: .short)
                                                snackbar.show()
                                            }
                                        }
                                    }
            ...
            let snackbar: CometChatSnackbar = CometChatSnackbar.init(message: "You can forward message only to 5 participants at a time.", duration: .short)
            snackbar.show()
        }
    }
}
