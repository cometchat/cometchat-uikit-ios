...
public final class SmartRepliesStyle: BaseStyle {
    ...
    private(set) var textFont = CometChatTheme.typography.subtitle1
    private(set) var textColor = CometChatTheme.palatte.accent900
    private(set) var textBackground = CometChatTheme.palatte.background
    private(set) var borderRadius = CometChatCornerStyle(cornerRadius: 18.0)
    private(set) var shadowColor = CometChatTheme.palatte.accent300
    // TODO: - Discuss
