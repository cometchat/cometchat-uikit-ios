...
@IBDesignable public class CometChatConversationListItem: UITableViewCell {
    ...
    @discardableResult
    @objc public func hide(title: Bool) -> CometChatConversationListItem {
        self.title.isHidden = title
        return self
    }
    
    @discardableResult
    @objc public func hide(subtitle: Bool) -> CometChatConversationListItem {
    ...
    @discardableResult
    public func set(conversationInputData: ConversationInputData?) -> CometChatConversationListItem {
        
        if let conversationInputData = conversationInputData {
...
//        }
        ...
        didSet {
            if let currentConversation = conversation {
                ...
                case .user:
                    ...
                    self.statusIndicator.set(borderWidth: 3).set(borderColor: CometChatTheme.palatte?.background ?? .white)
                        ...
                        if currentMessage.deletedAt > 0.0 {
                            ...
                            case .custom where currentConversation.conversationType == .user:
                                ...
                                if let customMessage = currentConversation.lastMessage as? CustomMessage {
                                    if customMessage.type == UIKitConstants.MessageTypeConstants.location {
                                        self.set(subTitle: "CUSTOM_MESSAGE_LOCATION".localize())
                                    }else if customMessage.type == UIKitConstants.MessageTypeConstants.poll {
                                        self.set(subTitle: "CUSTOM_MESSAGE_POLL".localize())
                                    }else if customMessage.type == UIKitConstants.MessageTypeConstants.sticker {
                                        self.set(subTitle: "CUSTOM_MESSAGE_STICKER".localize())
                                    }else if customMessage.type == UIKitConstants.MessageTypeConstants.whiteboard {
                                        self.set(subTitle: "CUSTOM_MESSAGE_WHITEBOARD".localize())
                                    }else if customMessage.type == UIKitConstants.MessageTypeConstants.document {
                                        self.set(subTitle: "CUSTOM_MESSAGE_DOCUMENT".localize())
                                    }else if customMessage.type == UIKitConstants.MessageTypeConstants.meeting {
                ...
                self.show(groupActions: true)
                self.show(deletedMessages: true)
            }
           
            configureConversationListItem()
            
            if let title = conversationInputData?.title {
            ...
            if let thumbnail = conversationInputData?.thumbnail {
                self.hide(avatar: !thumbnail)
            }
            
            if let timestamp = conversationInputData?.timestamp {
                self.hide(time: !timestamp)
            }
    ...
    @objc func onItemLongClick(sender: UITapGestureRecognizer){
        if sender.state == .began {
            if let conversation = conversation {
                CometChatConversationEvents.emitOnItemClick(converation: conversation, index: nil)
            }
        }
    }
    ...
    private func configureConversationListItem() {
        ...
        if let configurations = configurations {
            ...
            if let configuration = conversationListItemConfiguration.last as? ConversationListItemConfiguration {
                ...
                set(conversationInputData: configuration.conversationInputData)
            }
    ...
    func parseSentimentAnalysis(forMessage: TextMessage){
        if let metaData = forMessage.metaData , let injected = metaData["@injected"] as? [String : Any], let cometChatExtension =  injected[ExtensionConstants.extensions] as? [String : Any], let sentimentAnalysisDictionary = cometChatExtension[ExtensionConstants.sentimentalAnalysis] as? [String : Any] {
